...
{
    half2 ds = reinterpret_cast<const half2*>(&scales.x)[0] * make_half2(1.0f, -1.0f); const int* sc = reinterpret_cast<const int*>(&scales.y);
...
#else
#define MOE_X_Q4_K_NEW 8
#define MOE_Y_Q4_K 32
#define NWARPS_Q4_K_MOE_NEW 2
#endif
...
__launch_bounds__(WARP_SIZE_GGUF* NWARPS_Q4_K, 2)
    ...
    moe_q4_K(const int* __restrict__ vx_qs,
            ...
            const int top_k) {
        const int mmq_x = MOE_X_Q4_K_NEW;
        const int mmq_y = MOE_Y_Q4_K;
        const int nwarps = NWARPS_Q4_K_MOE_NEW;
