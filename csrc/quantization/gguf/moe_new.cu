...
#pragma unroll
                 ...
                 if(blockIdx.x == 0 && blockIdx.y == 14)
                 {
                     printf("loading val to %d, %d from %d, %d\n", (threadIdx.y + i), kqs%WARP_SIZE_GGUF, col_y_eff, threadIdx.x%QI8_1);
                 }
                 ...
                 if(blockIdx.x == 0 && blockIdx.y == 14)
                 {
                     printf("%d, %d, loading scale 1 to %d, %d from %d, %d\n", threadIdx.x, threadIdx.y, ((lane_id%4)*2), kby, col_y_eff, block_x);
                 }
                 ...
                 if(blockIdx.x == 0 && blockIdx.y == 14)
                 {
                     ...
                     __syncthreads();
                     if(threadIdx.x == 1 && threadIdx.y == 1 && blockIdx.x == 0 && (col_dst.x == 0 || col_dst.y==0))
                     {
                         printf("-----Y-------\n");
...
void ggml_moe_q4_K_q8_1_cuda_new(
...
// Add these at the end of your .cu file
template void ggml_moe_q4_K_q8_1_cuda_new<half>(
    const void*, const int*, const half2*,
    half*, const int*, const int*, const int*,
    const int, const int, const int, const int,
    const int, const int, const int, const int,
    const int, cudaStream_t);

template void ggml_moe_q4_K_q8_1_cuda_new<float>(
    const void*, const int*, const half2*,
